  ...
  Ownable
{
  ...
  string base_url = 'https://filerights.id-chain.net/nft/';
  ...
  */
  ...
  function tokenIdByHash(string calldata hash) external view returns(uint256 tokenId){
      return hashToId[hash];
  }
  
  function tokenHashById(uint256 _tokenId) external view returns(string memory hash){
      return IdToHash[_tokenId];
  }
  
  function tokenOwnerByHash(string calldata hash) external view returns(address tokenOwner){
      return  idToOwner[hashToId[hash]];
  }

  
  function mint(
    address _to,
    string calldata file_hash
  ) 
    external
    payable
    returns(uint256 t_id)
  {
    require(msg.value == mint_price, "you should send value of mint_price");
    require(hashToId[file_hash] == 0, "token for this file_hash already minted");
    ...
    string memory hash = file_hash;
    string memory _uri = base_url;
    _uri = concatenate(_uri,hash);
    _uri = concatenate(_uri,postfix);
    
    
    super._mint(_to, _tokenId);
    super._setTokenUri(_tokenId, _uri);
    hashToId[file_hash] = _tokenId;
    IdToHash[_tokenId] = file_hash;
    next_token_id++;
    return _tokenId;
  }
    ...
    external
  {
    ...
    if (bytes(IdToHash[_tokenId]).length != 0){
      string memory hash = IdToHash[_tokenId];
